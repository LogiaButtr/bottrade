//+------------------------------------------------------------------+
//|                                 TrendFollowingBreakoutEA_v3.mq5  |
//|                                                                  |
//| V3 CHANGELOG (on top of v2's HTF/BE/trailing/partial/filters):   |
//|  + Max total drawdown circuit breaker (from equity peak)         |
//|  + Margin pre-check before sending any order                     |
//|  + Broker stop-level / freeze-level distance check                |
//|  + Retry logic on requote/timeout/price-changed errors            |
//|  + Volatility-adaptive position sizing (shrinks risk in abnormal |
//|    volatility instead of using a flat risk % always)              |
//|  + CSV trade journal (every deal logged automatically)            |
//|                                                                  |
//| Scope note: correlation filter, multi-symbol portfolio mode, and |
//| an on-chart GUI dashboard were intentionally left out of this    |
//| pass to keep the EA a single, reviewable file.                    |
//+------------------------------------------------------------------+
