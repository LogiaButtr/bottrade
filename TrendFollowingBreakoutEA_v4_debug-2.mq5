...
input int    InpMaxSpreadPoints      = 150;  // CHECK your symbol's real spread (Market Watch -> Symbol -> Specification) and set this comfortably above it - 40 is too tight for many symbols (gold, indices, JPY pairs, crypto)
...
input group "===== Debug (diagnose 'no trades at all') ====="
input bool   InpDebugMode            = true; // count how many times each entry gate passes/blocks, print summary at the end
...
long     ExtDbgBarsChecked;
long     ExtDbgBlockedByOpenPosition;
long     ExtDbgBlockedByMaxDrawdown;
long     ExtDbgBlockedByDailyLimit;
long     ExtDbgBlockedByDailyTradeCount;
long     ExtDbgBlockedBySession;
long     ExtDbgBlockedBySpread;
long     ExtDbgBlockedByNews;
long     ExtDbgNoRegime;
long     ExtDbgRegimeButNoBreakout;
long     ExtDbgBlockedBySqueeze;
long     ExtDbgBlockedByHTF;
long     ExtDbgEntriesAttempted;
...
void OnDeinit(const int reason)
  {
   ...
   if(InpDebugMode)
     {
      Print("========== DEBUG SUMMARY (why entries were or weren't taken) ==========");
      Print("Total new-bar checks:                    ", ExtDbgBarsChecked);
      Print("Blocked - a position was already open:   ", ExtDbgBlockedByOpenPosition);
      Print("Blocked - max drawdown circuit tripped:  ", ExtDbgBlockedByMaxDrawdown);
      Print("Blocked - daily loss limit hit:          ", ExtDbgBlockedByDailyLimit);
      Print("Blocked - max trades/day reached:        ", ExtDbgBlockedByDailyTradeCount);
      Print("Blocked - outside session window:        ", ExtDbgBlockedBySession);
      Print("Blocked - spread too wide:                ", ExtDbgBlockedBySpread);
      Print("Blocked - high-impact news window:       ", ExtDbgBlockedByNews);
      Print("No regime at all (ADX/EMA condition):    ", ExtDbgNoRegime);
      Print("Regime OK but no breakout that bar:      ", ExtDbgRegimeButNoBreakout);
      Print("Entry candidates found (regime+breakout):", ExtDbgEntriesAttempted);
      Print("  -> of those, blocked by squeeze filter:", ExtDbgBlockedBySqueeze);
      Print("  -> of those, blocked by HTF confirm:   ", ExtDbgBlockedByHTF);
      Print("=========================================================================");

      if(ExtDbgBarsChecked == 0)
         Print("DIAGNOSIS: OnTick's new-bar logic never ran at all. Check the test date range and that the symbol has ANY price history for PERIOD_CURRENT in that range.");
      else if(ExtDbgNoRegime == ExtDbgBarsChecked)
         Print("DIAGNOSIS: The regime filter (EMA200 + ADX>=", InpADXThreshold, ") was NEVER true. Try lowering InpADXThreshold, or confirm the symbol has 200+ bars of PERIOD_CURRENT history before the test start date.");
      else if(ExtDbgEntriesAttempted == 0)
         Print("DIAGNOSIS: Regime was true sometimes, but the Donchian breakout (InpBreakoutPeriod=", InpBreakoutPeriod, ", buffer=", InpBreakoutBufferPoints, " pts) never fired alongside it. Try a longer test period, or reduce InpBreakoutBufferPoints.");
      else if(ExtDbgBlockedBySqueeze >= ExtDbgEntriesAttempted && InpUseSqueezeFilter)
         Print("DIAGNOSIS: Every entry candidate was blocked by the squeeze filter. Try InpUseSqueezeFilter=false, or check the symbol has enough history for InpSqueezeBaselineLookback+InpSqueezeRecentBars bars.");
      else if(ExtDbgBlockedByHTF >= (ExtDbgEntriesAttempted - ExtDbgBlockedBySqueeze) && InpUseHTFConfirm)
         Print("DIAGNOSIS: Every remaining entry candidate was blocked by the HTF confirmation. Confirm H4 (or your chosen InpHTFTimeframe) history is downloaded for this symbol, or try InpUseHTFConfirm=false.");
     }
  }
...
void OnTick()
  {
   ...
   if(HasOpenPosition())
     {
      if(InpDebugMode) ExtDbgBlockedByOpenPosition++;
      return;
     }

   if(InpUseMaxDrawdownLimit && ExtMaxDrawdownHit)
     {
      if(InpDebugMode) ExtDbgBlockedByMaxDrawdown++;
      return;
     }

   if(InpUseDailyLossLimit && ExtDailyLimitHit)
     {
      if(InpDebugMode) ExtDbgBlockedByDailyLimit++;
      return;
     }

   if(InpUseDailyLossLimit && InpMaxTradesPerDay > 0 && ExtTradesToday >= InpMaxTradesPerDay)
     {
      if(InpDebugMode) ExtDbgBlockedByDailyTradeCount++;
      return;
     }

   if(InpUseSessionFilter && !IsWithinSession())
     {
      if(InpDebugMode) ExtDbgBlockedBySession++;
      return;
     }
   ...
   if(InpUseSpreadFilter && !IsSpreadAcceptable())
     {
      if(InpDebugMode)
        {
         ExtDbgBlockedBySpread++;
         if(ExtDbgBlockedBySpread == 1)
            Print("DEBUG: first spread block - current spread = ", SymbolInfoInteger(_Symbol, SYMBOL_SPREAD),
                  " points, InpMaxSpreadPoints = ", InpMaxSpreadPoints, ". Raise InpMaxSpreadPoints if this is expected for your symbol.");
        }
      return;
     }
   ...
   if(InpUseNewsFilter && IsHighImpactNewsWindow())
     {
      if(InpDebugMode) ExtDbgBlockedByNews++;
      return;
     }
   ...
   if(!isUptrendRegime && !isDowntrendRegime)
     {
      if(InpDebugMode) ExtDbgNoRegime++;
      return;
     }

   if(isUptrendRegime && CheckBullishBreakout())
     {
      if(InpDebugMode) ExtDbgEntriesAttempted++;

      if(!WasRecentSqueeze())
        {
         if(InpDebugMode) ExtDbgBlockedBySqueeze++;
         return;
        }

      if(InpUseHTFConfirm && !IsHTFBullish())
        {
         if(InpDebugMode) ExtDbgBlockedByHTF++;
         return;
        }
